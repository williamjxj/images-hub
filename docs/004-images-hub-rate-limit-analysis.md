...
```typescript
// Pseudo-code
function getOptimalPerPage(provider: string, rateLimitRemaining: number): number {
  ...
  switch (provider) {
    case 'unsplash': return 30; // Max out Unsplash
    case 'pexels': return 60;   // High but safe
    case 'pixabay': return 100; // High but safe
  }
}
```
...
export async function searchImages(
  ...
  const perPageMap = providers.reduce((acc, provider) => {
